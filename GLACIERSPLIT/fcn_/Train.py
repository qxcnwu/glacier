...
from MadeRecord_r import tfrecords_reader_dataset
...
train_record="fcn_record/test.tfrecord"
test_record="fcn_record/test.tfrecord"
batch_size = 2
epoch=1000
...
def accu(y_true,y_pred):
    ...
    y_true=K.squeeze(K.squeeze(K.one_hot(K.cast(y_true,'int32'),num_classes=2),axis=3),axis=3)
    y_pred=K.one_hot(K.argmax(y_pred),num_classes=2)
...
for Ti in epochs:
    ...
    reduce_lr = CosineAnnealing(eta_max=1, eta_min=0.00001, total_iteration=Ti * (int(4*0.8) // batch_size), iteration=3, verbose=1)
    model.fit_generator(train_data, steps_per_epoch=int(4 * 0.8 / batch_size), epochs=Ti,callbacks=[model_checkpoint, reduce_lr], shuffle=True,validation_data=valid_data, validation_steps=int(4 * 0.2 / batch_size))